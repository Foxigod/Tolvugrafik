...
function render()
{
    ...
    rotDay += 1.0;
    rotYear += deltaRotYear;        // not the correct value, but looks better!
    ...
    // teikna "tunglið"
    var mvt = mult( mv, rotateY( 28.0*rotYear ) );
    mvt = mult( mvt, translate( 1.8, 0.0, 0.0 ) );
    mvt = mult( mvt, scalem( 0.4, 0.4, 0.4 ) );
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mvt));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );

    // teikna gervi-"tunglið"
    var gmvt = mult( mvt, rotateX( 30.0*rotYear*5. ) );
    gmvt = mult( gmvt, translate( 0.0, 0.9, 0.0 ) );
    gmvt = mult( gmvt, scalem( 0.15, 0.15, 0.15 ) );
    gl.uniformMatrix4fv(matrixLoc, false, flatten(gmvt));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );
