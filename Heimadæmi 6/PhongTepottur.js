...
var diffuseColorInteger = 724;
// var materialDiffuse = vec4( 1.0, 0.8, 0.0, 1.0 );
var materialDiffuse = diffuseFromInt(diffuseColorInteger);
...
function diffuseFromInt(integer){
    return vec4(Math.floor(integer/100)*0.1+0.1, Math.floor((integer%100)/10)*0.1+0.1, Math.floor(integer%10)*0.1+0.1, 1.0);
}
    ...
    window.addEventListener("keydown", function(e){
        switch( e.keyCode ) {
            case 38:    // up arrow
                ...
                break;
            case 40:    // down arrow
                threshold = Math.max(threshold-0.05, 0.0)
                gl.uniform1f( thresholdLoc, threshold );
                break;

            case 37:    // left arrow
                diffuseColorInteger = Math.max(diffuseColorInteger-1, 0);
                materialDiffuse = diffuseFromInt(diffuseColorInteger);
                diffuseProduct = mult(lightDiffuse, materialDiffuse);
                gl.uniform4fv( diffuseLoc, flatten(diffuseProduct) );
                break;
            case 39:    // right arrow
                diffuseColorInteger = Math.min(diffuseColorInteger+1, 999);
                materialDiffuse = diffuseFromInt(diffuseColorInteger);
                diffuseProduct = mult(lightDiffuse, materialDiffuse);
                gl.uniform4fv( diffuseLoc, flatten(diffuseProduct) );
                break;
