...
// Hnútar veggsins
var vertices = [
    vec4( -lengdGangs/2,  0.0, 0.0, 1.0 ),
    vec4(  lengdGangs/2,  0.0, 0.0, 1.0 ),
    vec4(  lengdGangs/2,  1.0, 0.0, 1.0 ),
    vec4(  lengdGangs/2,  1.0, 0.0, 1.0 ),
    vec4( -lengdGangs/2,  1.0, 0.0, 1.0 ),
    vec4( -lengdGangs/2,  0.0, 0.0, 1.0 ),
// Hnútar gólfsins (strax á eftir)
    vec4( -lengdGangs/2,  0.0, 2*userZPos, 1.0 ),
    vec4(  lengdGangs/2,  0.0, 2*userZPos, 1.0 ),
    vec4(  lengdGangs/2,  0.0,  0.0, 1.0 ),
    vec4(  lengdGangs/2,  0.0,  0.0, 1.0 ),
    vec4( -lengdGangs/2,  0.0,  0.0, 1.0 ),
    vec4( -lengdGangs/2,  0.0, 2*userZPos, 1.0 ),
    // Hnútar hins veggsins
    vec4( -lengdGangs/2,  0.0, 2*userZPos, 1.0 ),
    vec4(  lengdGangs/2,  0.0, 2*userZPos, 1.0 ),
    vec4(  lengdGangs/2,  1.0, 2*userZPos, 1.0 ),
    vec4(  lengdGangs/2,  1.0, 2*userZPos, 1.0 ),
    vec4( -lengdGangs/2,  1.0, 2*userZPos, 1.0 ),
    vec4( -lengdGangs/2,  0.0, 2*userZPos, 1.0 )
];
...
// Mynsturhnit fyrir gólf
    vec2(  0.0,  0.0 ),
    vec2( lengdGangs,  0.0 ),
    vec2( lengdGangs, 2*userZPos ),
    vec2( lengdGangs, 2*userZPos ),
    vec2(  0.0, 2*userZPos ),
    vec2(  0.0,  0.0 ),
    // Mynsturhnit fyrir vegg endurtekin
    vec2(  0.0, 0.0 ),
    vec2( lengdGangs, 0.0 ),
    vec2( lengdGangs, 1.0 ),
    vec2( lengdGangs, 1.0 ),
...
window.onload = function init() {
    ...
    // Event listener for keyboard
     window.addEventListener("keydown", function(e){
         ...
         if (userZPos > 2*userStartZPos-0.22) userZPos = 2*userStartZPos-0.22;
         if (userZPos < 0.22) userZPos = 0.22;
     }  );  
