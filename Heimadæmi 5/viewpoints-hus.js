...
window.onload = function init()
{
    ...
    canvas.addEventListener("mousemove", function(e){
        if(movement) {
    	    spinY = ( spinY + (e.clientX - origX)/100 ) % 360;
            spinX = ( spinX + (e.clientY - origY)/100 ) % 360;
            origX = e.clientX;
            origY = e.clientY;
        }
    } );
    ...
    window.addEventListener("keydown", function(e){
        switch( e.keyCode ) {
            ...
            case 56:	// 8: from beside the car
                ...
                break;
            case 57:	// 9: from ontop the house
                view = 9;
                document.getElementById("Viewpoint").innerHTML = "9: Ofan á húsinu";
                break;
            case 48:	// 0: constant height
                view = 0;
                document.getElementById("Viewpoint").innerHTML = "0: Föst hæð";
                break;
            
            case 38:    // up arrow
                if (view == 0){
                    eye_xpos -= Math.cos(spinY);
                    eye_ypos += Math.sin(spinY);
                    break;
                }
                height += 2.0;
                document.getElementById("Height").innerHTML = "Viðbótarhæð: "+ height;
...
function render()
{
    ...
    case 0:
        // Distant and stationary viewpoint
        var tempvec = vec3(eye_xpos, eye_ypos, 10.0);
	    mv = lookAt( tempvec, add(tempvec, vec3(-50*Math.cos(spinY), 50*Math.sin(spinY), 0.0)), vec3(0.0, 0.0, 1.0) );
	    drawScenery( mv );
	    mv = mult( mv, translate( carXPos, carYPos, 0.0 ) );
	    mv = mult( mv, rotateZ( -carDirection ) ) ;
	    drawCar( mv );
